...
void MainWindow::AddSolvers(bool isInteger)
{
    if (isInteger) {
        ...
        ui->comboSolver->addItem("GLPK");

        ui->checkBox_clp->hide();
        ui->checkBox_glop->hide();

        ui->checkBox_cbc->show();


    } else {
        ui->comboSolver->addItem("GLOP");
        ui->comboSolver->addItem("CLP");
        ui->comboSolver->addItem("GLPK");

        ui->checkBox_cbc->hide();

        ui->checkBox_glop->show();
        ui->checkBox_clp->show();
...
void MainWindow::on_btnSolve_clicked()
{
    ...
    ui->txtTime->setText(QString::number(program->get_time()) + " мс");
}
...
{
    program->SetIterations(ui->txtIterations->text().toInt());
    if(ui->checkBox_glop->isChecked()) {
        ui->textEdit_output->append("GLOP :\nОтвет - " + QString::number(program->solve("GLOP", Parameters()))
                                    + "\nСреднее время решения - " + QString::number(program->get_time()) + " мс");
    }
    if(ui->checkBox_clp->isChecked()) {
        ui->textEdit_output->append("CLP :\nОтвет - " + QString::number(program->solve("CLP", Parameters()))
                                    + "\nСреднее время решения - " + QString::number(program->get_time()) + " мс");
    }
    if(ui->checkBox_cbc->isChecked()) {
        ui->textEdit_output->append("CBC :\nОтвет - " + QString::number(program->solve("CBC", Parameters()))
                                    + "\nСреднее время решения - " + QString::number(program->get_time()) + " мс");
    }
}
